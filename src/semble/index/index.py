...
class SembleIndex:
    ...
    def __init__(
        ...
        self._by_id: dict[str, Chunk] = dict(zip(chunk_ids, chunks))
    ...
    def _populate_mapping(self) -> tuple[dict[str, list[str]], dict[str, list[str]]]:
        language_to_id: dict[str, list[str]] = defaultdict(list)
        file_to_id: dict[str, list[str]] = defaultdict(list)
        for cid, chunk in zip(self.chunk_ids, self.chunks):
            if chunk.language:
                language_to_id[chunk.language].append(cid)
            file_to_id[chunk.file_path].append(cid)
    ...
    def from_path(
        ...
        if cfg.cache.enabled:
            ...
            if cache_mgr.is_disk_valid(str(path), current_hashes=file_hashes, extensions=ext_set):
                logger.info("Loading cached index for %s...", path)
                cached = cache_mgr.load_from_disk(str(path))
                if cached is not None:
                    chunks, chunk_ids, vs, sparse_idx = cached
                    logger.info("Loaded cached index: %d chunks", len(chunks))
                    return cls(
                        resolved_model, sparse_idx, vs, chunks, chunk_ids,
                        root=path, reranker=reranker, config=cfg,
                    )

        logger.info("Building index for %s...", path)
        vs = create_vector_store(cfg.vector_store, resolved_model.dim)
        sparse_idx, vs, chunks, chunk_ids = create_index_from_path(
            path,
        ...
        if cfg.cache.enabled:
            ...
            ext_set = frozenset(get_extensions(include_text_files, extensions))
            file_hashes = HashTracker.compute_hashes(path, ext_set)
            cache_mgr.save_to_disk(str(path), index, file_hashes)
    ...
    def from_git(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            return cls(
                resolved_model, sparse_idx, vs, chunks, chunk_ids,
                root=resolved_path, reranker=reranker, config=cfg,
            )
    ...
    def find_related(self, source: Chunk | SearchResult, *, top_k: int = 5) -> list[SearchResult]:
        ...
        selector_ids = self._get_selector_ids(filter_languages=[target.language]) if target.language else None
        query_emb = self.model.encode([target.content])
        hits = self._semantic_index.query(
            LOCAL_NAMESPACE, query_emb, k=top_k + 1, selector_ids=selector_ids,
        )
        results: list[SearchResult] = []
        for cid, score in hits:
            chunk = self._by_id.get(cid)
            if chunk is None or chunk == target:
                continue
    ...
    def _get_selector_ids(
        ...
        if not selector:
            return None
        # Deduplicate while preserving order so callers see a stable iteration.
        seen: set[str] = set()
        deduped = []
        for cid in selector:
            if cid not in seen:
    ...
    def search(
        ...
        if not self.chunks or not query.strip():
            return []
        selector_ids = self._get_selector_ids(filter_languages, filter_paths)
